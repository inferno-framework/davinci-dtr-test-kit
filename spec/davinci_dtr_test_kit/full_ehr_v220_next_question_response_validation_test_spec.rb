...
RSpec.describe DaVinciDTRTestKit::DTRFullEHRV220NextQuestionResponseValidationTest do # rubocop:disable RSpec/SpecFilePathFormat
  ...
  def create_nq_request(response_body, url: next_question_url)
    repo_create(
      :request,
      direction: 'incoming',
      url:,
  ...
  def bare_qr_json(status: 'completed')
    { resourceType: 'QuestionnaireResponse', status: }.to_json
  end

  def wrapped_qr_json(status: 'completed')
    {
      resourceType: 'Parameters',
      parameter: [{ name: 'return', resource: { resourceType: 'QuestionnaireResponse', status: } }]
    }.to_json
  end
  ...
  it 'fails and does not attempt validation when the response is not valid JSON' do
  ...
  it 'validates a bare QuestionnaireResponse response, the conformant shape' do
    stub_valid_resource
  ...
  it 'also accepts a QuestionnaireResponse wrapped in a Parameters `return` parameter' do
    stub_valid_resource
    create_nq_request(wrapped_qr_json)
  ...
  it 'validates the extracted QuestionnaireResponse against the adaptive response profile' do
    create_nq_request(bare_qr_json)
    validated_with = nil
  ...
  it 'fails and does not attempt validation when the response contains no QuestionnaireResponse' do
    ...
    expect(result.result).to eq('fail')
    expect(messages_for(result)).to include('(Request 1) Response does not contain a QuestionnaireResponse')
  end

  it 'fails when the response is Parameters without a `return` parameter' do
    create_nq_request({ resourceType: 'Parameters', parameter: [] }.to_json)
