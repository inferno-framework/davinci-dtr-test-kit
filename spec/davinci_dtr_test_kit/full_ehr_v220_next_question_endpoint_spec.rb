  ...
  describe 'When responding' do
    ...
    context 'when using an input-based response template' do
      ...
      it 'returns 400 with OperationOutcome when no input value is provided' do
        run(nq_input_test, client_id:, qp_response_template: adaptive_questionnaire_package_json)
        post_qp_with_token
        post_nq_with_token
      ...
      it 'returns 400 with OperationOutcome when the input is not valid FHIR' do
        run(nq_input_test, client_id:,
                           qp_response_template: adaptive_questionnaire_package_json,
        ...
        parsed = JSON.parse(last_response.body)
        expect(parsed['resourceType']).to eq('OperationOutcome')
        expect(parsed['issue'].first['details']['text']).to include('does not contain valid JSON')
      end

      it 'returns 400 with OperationOutcome when the input is a non-Questionnaire FHIR resource' do
        run(nq_input_test, client_id:,
                           qp_response_template: adaptive_questionnaire_package_json,
        ...
        parsed = JSON.parse(last_response.body)
        expect(parsed['resourceType']).to eq('OperationOutcome')
        expect(parsed['issue'].first['details']['text']).to include('Expected Questionnaire, got')
      end
    end
