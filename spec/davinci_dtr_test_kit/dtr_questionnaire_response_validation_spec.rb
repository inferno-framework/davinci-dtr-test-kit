RSpec.describe DaVinciDTRTestKit::DTRQuestionnaireResponseValidation do
  ...
  describe '#validate_questionnaire_pre_population' do
    ...
    context('with a QuestionnaireResponse with missing an answer missing origin.source') do
      let(:questionnaire_response_missing_origin) do
        File.read(File.join(__dir__, '..', 'fixtures', 'questionnaire_response_missing_origin.json'))
      end

      it 'fails' do
        allow_any_instance_of(test).to receive(:assert_valid_resource).and_return(true)
        repo_create(:request, name: 'questionnaire_response_save', url: questionnaire_response_url,
                              request_body: questionnaire_response_missing_origin, test_session_id: test_session.id)
        result = run(test)
        expect(result.result).to eq('fail')
      end
    end
