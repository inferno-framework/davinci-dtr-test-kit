module DaVinciDTRTestKit
  module CQLTest
    ...
    def extract_questionnaires(responses)
      questionnaires = responses.flat_map { |response|
        if response.class == Inferno::Entities::Request
          ...
          # }
        elsif response.class == FHIR::Parameters
          response.parameter.filter_map { |param|
            param if param.resource.entry.resource.resourceType == "Questionnaire"
          }
        elsif response.class == FHIR::QuestionnaireResponse 
          response.contained.filter_map { |contained|
    ...
    def check_population_extensions(questionnaire_bundle)
      found_bundle = found_questionnaire = false
      # are extensions present in any questionnaire?
      found_min_launch_context = found_min_variable = found_min_pop_context = 
      found_min_init_expression = found_min_candidate_expression = found_min_context_expression = found_min_cqf_lib = false
      questionnaire_bundle.parameter.each { |param|
