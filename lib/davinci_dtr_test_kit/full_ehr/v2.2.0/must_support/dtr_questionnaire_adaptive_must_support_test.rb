...
module DaVinciDTRTestKit
  class DTRFullEHRV220QuestionnaireAdaptiveMustSupportTest < Inferno::Test
    include QuestionnaireHelper

    MUST_SUPPORT_METADATA = GeneratedProfileMetadata.for('v2.2.0', 'dtr_questionnaire_adapt')

    id :dtr_full_ehr_v220_questionnaire_adaptive_must_support
    title 'Client supports adaptive Questionnaire must support elements and extensions'
    description %(
      ...
      ave been observed across all completed adaptive Questionnaires during previous tests.
      This does not include must support elements flagged within the
      [DTR Base Questionnaire](https://hl7.org/fhir/us/davinci-dtr/2.2.0/en/StructureDefinition-dtr-base-questionnaire.html)
      profile. Note that adaptive Questionnaires from test scenarios where the Questionnaire
    ...
    run do
      ...
      skip_if questionnaires.blank?, 'No adaptive Questionnaires found to evaluate.'
