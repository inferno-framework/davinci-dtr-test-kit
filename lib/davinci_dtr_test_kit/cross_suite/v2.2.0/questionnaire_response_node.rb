module DaVinciDTRTestKit
  ...
  class QuestionnaireResponseNode
    ...
    def initialize(kind:, payload:, parent: nil, index_in_parent: nil)
      @kind = kind
      @payload = payload
      @parent = parent
    ...
    # This node followed by its descendants, in document order. Memoized, like the lists below and
    # `children`, because a search asks many nodes for their subtree and each list is built from the
    # lists of the node's children, which would otherwise be rebuilt at every level.
    def self_and_descendants
      @self_and_descendants ||= [self] + children.flat_map(&:self_and_descendants)
    end
    ...
    def self_and_descendants_reversed
      @self_and_descendants_reversed ||= [self] + children.reverse.flat_map(&:self_and_descendants_reversed)
    end
